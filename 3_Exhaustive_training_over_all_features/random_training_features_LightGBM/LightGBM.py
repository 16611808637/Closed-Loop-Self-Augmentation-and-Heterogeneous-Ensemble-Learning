...
import os
# ==========================================
# 1. Configuration Section (Fixed Pm as mandatory first column)
# ==========================================
MAX_FEATURES = 9
TRAIN_PATH = 'train.csv'
TEST_PATH = 'test.csv'
OUTPUT_LOG = 'LGBM_fast_log.txt'
TASK_STATE_FILE = "task_done_lgb.json"
FIXED_COLS = 1  # Fixed first column (Pm) as mandatory feature for all combinations
# Parallel resources, slurm -n 32
NUM_WORKERS = 32
LGB_N_JOBS = 1   # Disable LGB internal threads, use external process pool
...
# ==========================================
# 3. Fixed Hyperparameters (Unchanged except n_jobs)
# ==========================================
params = {
    'random_state': 20,
    'verbosity': -1,
...
# ==========================================
# ✅ Breakpoint auto-parse function: Find last completed position from log
# ==========================================
...
def single_combo_task(task_key, n, combo_idx, selected_optional_tuple, fixed_cols):
    ...
    log_msg = (
        ...
        f"Column indexes used: {final_cols}\n"
...
def main():
    ...
    # ==========================================
    # 2. Load Dataset (Loaded only once)
    # ==========================================
    ...
    if MAX_FEATURES > total_feature_cols:
        raise ValueError(f"The dataset only contains {total_feature_cols} feature columns")
    ...
    end_total = time.time()
    with open(OUTPUT_LOG, 'a', encoding='utf-8') as f:
        f.write(f"\nAll tasks completed! Total execution time: {end_total - start_total:.2f} seconds\n")
    print(f"\n🎉 All combinations have been traversed! Total execution time this run: {end_total - start_total:.2f} s")
